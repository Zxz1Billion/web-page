...
> `assets/survival` extract; nothing is guessed. The extracts caught **three** structural changes
> vs. older data, all corrected here:
> 1. armor moved to `itemtypes/wearable/seraph/armor.json` (durability numbers unchanged);
> 2. the forge rework split the spear recipe into per-metal files (`spear-copper/bronze/blackbronze/iron/meteoriciron.json`);
> 3. animals moved from `entities/land/` to `entities/animal/**`, were renamed `-adult`/`-baby`, the
...
**5. Armor (`armor.json`)** — every `durabilityByType` entry doubled (improvised/cloth/leather,
...
**6. Forging (`forgeyields.json`)** — adds `output.stacksize = 2` to the anvil smithing recipes
...
patch files + `modinfo.json`. It is version-independent (handles per-variant `quantityByType`,
locates the `harvestable` behavior index per entity, etc.), so it tracks any 1.22.3 changes
automatically. Tunable multipliers are constants at the top of the file. Crop yields are disabled
(`ENABLE_CROPS = False`) to match this mod; flip it to re-enable that logic.
