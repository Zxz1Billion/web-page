...
const LAYOUTS = [
  ...
  { name: "The Atrium House", sub: "13×12 · ROOMS ROUND A COURT",
    ...
    W: 13, H: 12,
    walls: [{ x1: 4, y1: 4, x2: 9, y2: 4 }, { x1: 4, y1: 8, x2: 9, y2: 8 }, { x1: 4, y1: 4, x2: 4, y2: 8 }, { x1: 9, y1: 4, x2: 9, y2: 8 }, { x1: 4, y1: 0, x2: 4, y2: 4 }, { x1: 9, y1: 0, x2: 9, y2: 4 }, { x1: 4, y1: 8, x2: 4, y2: 12 }, { x1: 9, y1: 8, x2: 9, y2: 12 }, { x1: 0, y1: 6, x2: 4, y2: 6 }, { x1: 9, y1: 6, x2: 13, y2: 6 }, { x1: 5.8, y1: 5.3, x2: 7.2, y2: 5.3 }, { x1: 5.8, y1: 6.7, x2: 7.2, y2: 6.7 }, { x1: 5.8, y1: 5.3, x2: 5.8, y2: 6.7 }, { x1: 7.2, y1: 5.3, x2: 7.2, y2: 6.7 }],
    rooms: [{ x: 0, y: 0, w: 4, h: 6, label: "Kitchen", dim: "4 × 6" }, { x: 0, y: 6, w: 4, h: 6, label: "Living Room", dim: "4 × 6" }, { x: 4, y: 0, w: 5, h: 4, label: "Dining", dim: "5 × 4" }, { x: 4, y: 4, w: 5, h: 1.2, label: "Atrium", dim: "impluvium" }, { x: 4, y: 8, w: 5, h: 4, label: "Hall", dim: "+ stairs" }, { x: 9, y: 0, w: 4, h: 6, label: "Store", dim: "4 × 6" }, { x: 9, y: 6, w: 4, h: 6, label: "Workshop", dim: "4 × 6" }],
  ...
  { name: "The Hall & Solar", sub: "OPEN HALL + SOLAR CROSS-WING",
    blurb: "A little manor in two parts: a tall open hall entered through a screens passage, with a cross-wing thrown forward holding a private solar over a parlour, the grand stair between.",
    outline: [[0, 0], [14, 0], [14, 13], [9, 13], [9, 9], [0, 9]],
    walls: [{ x1: 9, y1: 0, x2: 9, y2: 13 }, { x1: 9, y1: 6, x2: 14, y2: 6 }, { x1: 3, y1: 0, x2: 3, y2: 9 }],
    rooms: [{ x: 0, y: 0, w: 3, h: 9, label: "Service", dim: "buttery" }, { x: 3, y: 0, w: 6, h: 9, label: "Great Hall", dim: "open" }, { x: 9, y: 0, w: 5, h: 6, label: "Solar", dim: "+ stairs" }, { x: 9, y: 6, w: 5, h: 7, label: "Parlour", dim: "5 × 7" }],
    doors: [{ x: 5, y: 9, axis: "h", len: 1.3, dir: -1 }, { x: 3, y: 5, axis: "v", len: 1.2, dir: 1 }, { x: 9, y: 3, axis: "v", len: 1.3, dir: 1 }, { x: 9, y: 8, axis: "v", len: 1.3, dir: 1 }, { x: 11.5, y: 6, axis: "h", len: 1.2, dir: 1 }],
    windows: [{ x: 0, y: 2, axis: "v", len: 1.0 }, { x: 0, y: 6, axis: "v", len: 1.0 }, { x: 5, y: 0, axis: "h", len: 1.8 }, { x: 3.5, y: 9, axis: "h", len: 1.2 }, { x: 14, y: 2.5, axis: "v", len: 1.3 }, { x: 14, y: 9.5, axis: "v", len: 1.4 }, { x: 11, y: 13, axis: "h", len: 1.4 }],
    stairs: [{ x: 12, y: 0.4, w: 1.8, h: 2.0, flow: "+x", label: "↑ solar · ↓ cellar" }] },
  ...
  { name: "The Cob Cottage", sub: "10×8 · ROUNDED CORNERS · ONE ROOM",
    blurb: "Hand-built in cob with soft rounded corners — one warm living-kitchen round the hearth, a curtained bed alcove off it, and a corner stair to the cellar. No draughty angles.",
    outline: [[1.5, 0], [8.5, 0], [10, 1.5], [10, 6.5], [8.5, 8], [1.5, 8], [0, 6.5], [0, 1.5]],
    walls: [{ x1: 6.5, y1: 0, x2: 6.5, y2: 3.5 }, { x1: 6.5, y1: 3.5, x2: 10, y2: 3.5 }],
    rooms: [{ x: 0, y: 2.6, w: 6.5, h: 5, label: "Living-Kitchen", dim: "hearth" }, { x: 6.5, y: 0, w: 3.5, h: 3.5, label: "Bed Alcove", dim: "" }],
    doors: [{ x: 2.5, y: 8, axis: "h", len: 1.3, dir: -1 }, { x: 6.5, y: 1.6, axis: "v", len: 1.0, dir: -1 }],
    windows: [{ x: 0, y: 4.5, axis: "v", len: 1.4 }, { x: 3.5, y: 0, axis: "h", len: 1.4 }, { x: 4, y: 8, axis: "h", len: 1.4 }, { x: 10, y: 5, axis: "v", len: 1.4 }],
    stairs: [{ x: 0.6, y: 0.6, w: 1.8, h: 1.8, flow: "+x", label: "↓ cellar" }] },
  ...
  { name: "The Windmill", sub: "9Ø · ROUND TOWER · MILLSTONES",
    blurb: "A round stone tower mill: the millstone vat dead centre on the stone floor, the meal spout below it, and a stair winding up the wall to the cap and sails.",
    outline: circle(4.5, 4.5, 4.2, 22),
    walls: [{ x1: 3.5, y1: 3.5, x2: 5.5, y2: 3.5 }, { x1: 3.5, y1: 5.5, x2: 5.5, y2: 5.5 }, { x1: 3.5, y1: 3.5, x2: 3.5, y2: 5.5 }, { x1: 5.5, y1: 3.5, x2: 5.5, y2: 5.5 }],
    rooms: [{ x: 1.5, y: 1.2, w: 6, h: 1.4, label: "Mill Floor", dim: "sack hoist" }, { x: 3.5, y: 3.5, w: 2, h: 2, label: "Stones", dim: "" }],
    doors: [{ x: 3.8, y: 8.7, axis: "h", len: 1.3, dir: -1 }, { x: 4.5, y: 5.5, axis: "h", len: 1.0, dir: 1 }],
    windows: [{ x: 0.3, y: 4, axis: "v", len: 1.0 }, { x: 8.7, y: 4, axis: "v", len: 1.0 }, { x: 4, y: 0.3, axis: "h", len: 1.0 }],
    stairs: [{ x: 6.7, y: 3.6, w: 1.6, h: 1.8, flow: "+y", label: "↑ cap" }] },
  ...
  // ---- COASTAL & HARBOUR ----------------------------------------------------
  { name: "The Lighthouse", sub: "OCTAGON TOWER + KEEPER'S COTTAGE",
    blurb: "An eight-sided sea-tower with the lamp gallery up top, joined to a single-room keeper's cottage. Build it tall on the headland and keep the wick trimmed.",
    outline: [[1.5, 1], [5.5, 1], [7, 2.5], [12, 2.5], [12, 5.5], [7, 5.5], [5.5, 7], [1.5, 7], [0, 5.5], [0, 2.5]],
    walls: [{ x1: 7, y1: 2.5, x2: 7, y2: 5.5 }],
    rooms: [{ x: 0, y: 1, w: 7, h: 6, label: "Lamp Tower", dim: "oil · stair" }, { x: 7, y: 2.5, w: 5, h: 3, label: "Keeper's Room", dim: "" }],
    doors: [{ x: 9, y: 5.5, axis: "h", len: 1.2, dir: -1 }, { x: 7, y: 4, axis: "v", len: 1.0, dir: -1 }],
    windows: [{ x: 0, y: 3.5, axis: "v", len: 0.8 }, { x: 2.5, y: 1, axis: "h", len: 0.8 }, { x: 12, y: 4, axis: "v", len: 1.2 }, { x: 9, y: 2.5, axis: "h", len: 1.2 }],
    stairs: [{ x: 0.8, y: 2.7, w: 1.6, h: 1.8, flow: "+y", label: "↑ lamp gallery" }] },
  ...
  { name: "The Sawmill", sub: "13×8 · OPEN SAW SHED · SAW-PIT",
    blurb: "An open-sided saw shed where the frame-saw rips the logs straight off the ramp, with a closed timber store for the cut planks and a sawyer's hut over the pit stair.",
    W: 13, H: 8,
    walls: [{ x1: 9, y1: 0, x2: 9, y2: 8 }, { x1: 9, y1: 4, x2: 13, y2: 4 }],
    rooms: [{ x: 0, y: 0, w: 9, h: 8, label: "Saw Shed", dim: "open · saw-pit" }, { x: 9, y: 0, w: 4, h: 4, label: "Sawyer's Hut", dim: "" }, { x: 9, y: 4, w: 4, h: 4, label: "Timber Store", dim: "" }],
    doors: [{ x: 0.8, y: 8, axis: "h", len: 2.3, dir: -1 }, { x: 4.5, y: 8, axis: "h", len: 2.3, dir: -1 }, { x: 11, y: 8, axis: "h", len: 1.3, dir: -1 }, { x: 9, y: 6, axis: "v", len: 1.3, dir: -1 }, { x: 11, y: 4, axis: "h", len: 1.2, dir: -1 }],
    windows: [{ x: 0, y: 2.5, axis: "v", len: 1.2 }, { x: 0, y: 5.5, axis: "v", len: 1.2 }, { x: 3, y: 0, axis: "h", len: 1.3 }, { x: 6, y: 0, axis: "h", len: 1.3 }, { x: 13, y: 2, axis: "v", len: 1.2 }, { x: 13, y: 6, axis: "v", len: 1.2 }],
    stairs: [{ x: 11.2, y: 0.4, w: 1.6, h: 1.8, flow: "+x", label: "↓ saw pit" }] },
